...
export class SearchbarComponent implements OnInit {
  ...
  filtersLoaded: Promise<boolean>; filtersLoaded2: Promise<boolean>;
  http: any;
  ...
  idSearch: String = "";
  active = 1;
  ability1: String; ability2: String; ability3: String; ability4: String;
  selectedProject: String = "Choose an Option";

  //find the moves chosen in the list.
  findMoves(ability1: String, ability2: String, ability3: String, ability4: String) {
    this.filtersLoaded2 = Promise.resolve(false);
    let moves: any[] = [ability1, ability2, ability3, ability4];
    let i = 5;
    moves.forEach(move => {
      this.dataserv.getPokemonMoves(move.toLowerCase()).subscribe((response: any) => {
        const a = new PokemonMoves(response.name, ifAccuracy(response.accuracy), response.type.name, ifAccuracy(response.power),
        ifAccuracy(response.pp), response.priority, response.target.name, ifAccuracy(response.effect_chance), ifAccuracy(response.effect_entries[0].effect),
    ...
    this.dataserv.getPokimonId(a).subscribe((response: any) => {
      ...
      try {
        ...
        if (response.types.length > 1) {
          ...
          const x = new PokemonModel(response.id, response.name, response.types[0].type.name,
            z, response.moves, response.stats, response.sprites.front_default,
            response.sprites.back_default, response.sprites.front_shiny, response.sprites.back_shiny);
            this.pokemons.push(x);  //push the code into pokemon string for html

            this.dataserv.getPokemonMoves(response.moves[0].move.name).subscribe((response1: any) => {
              if (response1) {
                console.log(response1);
                const m = new PokemonMoves(response1.name, response1.accuracy, response1.type.name, response1.power,
                  response1.pp, response1.priority, response1.target.name, response1.effect_chance, response1.effect_entries[0].effect,
              ...
              const x = new PokemonModel(response.id, response.name, response.types[0].type.name,
                "", response.moves, response.stats, response.sprites.front_default,
                response.sprites.back_default, response.sprites.front_shiny, response.sprites.back_shiny);
                //We are also creating a movies array list of all the moves available to this object.
                this.pokemons.push(x); //push the code into pokemon string for html
                this.dataserv.getPokemonMoves(response.moves[0].move.name).subscribe((response1: any) => {
                ...
                this.numSearch = null; this.idSearch = null;
          ...
          findName(n: String) {
            this.dataserv.getSpecificPokimon(n.toLowerCase()).subscribe((response: any) => {
              if (response) {
                //do we want to do anything if we get a successful response? Place that here.
              }
              try {
          ...
          const x = new PokemonModel(response.id, response.name, response.types[0].type.name,
            ...
            this.dataserv.getPokemonMoves(response.moves[0].move.name).subscribe((response1: any) => {
              ...
              const x = new PokemonModel(response.id, response.name, response.types[0].type.name,
                ...
                document.body.classList.add(this.pokemons[0].Type1.toLowerCase());
                this.dataserv.displayShiny = false;
                this.idSearch = null; this.numSearch = null;
                this.filtersLoaded = Promise.resolve(true);
                console.log(this.pokemons);
                console.log(this.moves);
              }
